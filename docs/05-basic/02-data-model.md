...
在下一个版本，属性的数据引用还将支持计算公式 (Formula)、字符串构建 (String Builder), 让数据引用更加灵活，来支持数据的标准化。计算公式是指该属性是基于其它属性的值计算出来的，比如智能电表的功率这个属性，是通过属性电流与属性电压相乘获得的，我们只需要配置计算的表达式即可。字符串构建是指定一个字符串构建的规则，基于几个字符串，构建出新的字符串。

今后属性的数据引用不仅能引用 TDengine TSDB 的数据，还能引用其它时序数据库或关系型数据库。
